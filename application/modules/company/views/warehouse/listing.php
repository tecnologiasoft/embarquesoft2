...
<html lang="en" >
    ...
    <head>
        <meta charset="utf-8" />
        <title>
            <?php echo $this->site_name; ?> | <?php echo $this->lang->line('title_warehouse_list'); ?>
        </title>
                                        ...
                                        <div class="col-xl-8 order-2 order-xl-1">
                                            <div class="form-group m-form__group row align-items-center">
                                                ...
                                                <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                                                    <a href="<?php echo site_url('company/warehouse/add');?>" class="btn btn-info m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                                                        <span>
                                                            <i class="la la-plus"></i> 
                                                            <span>
                                                                <?php echo $this->lang->line('label_add_warehouse'); ?>
                                                            </span>
                                                        </span>
                                                    </a>
        ...
        <script type="text/javascript">
            ...
            var t = $(".m_datatable").mDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: "<?php echo site_url('company/warehouse/ajax_list')?>"
                        }
                    },
                    pageSize: 10,
                    saveState: {
                ...
                },
                {
                    ...
                    overflow: "visible",
                    template: function(t) {
                        /* \t\t\t\t\t\t<a href="<?php echo base_url()."company/warehouse/view/";?>'+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-success m-btn--icon m-btn--icon-only m-btn--pill">\t\t\t\t\t\t\t<i class="la la-eye"></i>\t\t\t\t\t\t</a> */
                        return '\t\t\t\t\t\t\t\t\t\t\t<a href="<?php echo base_url()."company/warehouse/edit/";?>'+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill">\t\t\t\t\t\t\t<i class="la la-edit"></i>\t\t\t\t\t\t</a>\t\t\t\t\t\t\t\t\t\t\t<a href="javascript:;" onclick="delete_warehouse('+t.id+')" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" >\t\t\t\t\t\t\t<i class="la la-trash"></i>\t\t\t\t\t\t</a>'
                    }
                }]
            }),
            ...
            function delete_warehouse(id) 
            {
                swal({   
                    ...
                    text: "<?php echo $this->lang->line('label_you_want_to_delete_warehouse') ?>",
                    type: "warning",   
                    ...
                    $.ajax({
                        url:  "<?php echo base_url(); ?>company/warehouse/delete/"+id,
                        type: "GET",

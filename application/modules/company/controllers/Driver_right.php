...
class Driver_right extends MYcom_Controller {

	private $view_folder = 'company/driver_right/';
	function __construct()
    {
        ...
        $this->load->model('company/driver_right_model');
    }
    ...
    /*List all driver_right by ajax call*/
    function ajax_list()
    {
        $AllPostData = $this->input->post();
        $list = $this->driver_right_model->get_datatables();
        $data = array();

        $output = array(
                    "meta" => array('page'=>$AllPostData['datatable']['pagination']['page'],'pages'=>$AllPostData['datatable']['pagination']['pages'],'perpage'=>$AllPostData['datatable']['pagination']['perpage'],'total'=>$this->driver_right_model->count_filtered(),'sort'=>'asc','field'=>'company_id'),
                    "data" => $list,
                );
    ...
    /*Add new driver_right*/
    function add()
    {
        $data = array();
        $data['branch_list'] = $this->driver_right_model->get_branch_list();
        $data['user_list'] = $this->driver_right_model->get_user_list();
        ...
        {   
            ...
            $already_register = $this->driver_right_model->check_param(array('user_id' => $this->input->post('user_id')));
            if(!empty($already_register)){
                $this->session->set_flashdata('err_msg1', $this->lang->line('text_driver_right_exist'));
            ...
            $id = $this->driver_right_model->add_driver_right($params);
 
            $this->session->set_flashdata('succ_msg1', $this->lang->line('text_driver_right_add_success'));
            redirect('company/driver_right/listing/','refresh');
        }
    }

  
    /*Update driver_right details*/
    function edit()
    {
        $driver_right_id = $this->uri->segment(4);

        $data = array();
        $driver_right_data = $data['result'] = $this->driver_right_model->get_driver_right_data($driver_right_id);
        if(empty($driver_right_data))
            redirect('company/driver_right/listing','refresh');
        ...
        else 
        {
            ...
            // Update driver_right details 
            $this->driver_right_model->update_driver_right($driver_right_id, $params);

            $this->session->set_flashdata('succ_msg1', $this->lang->line('text_driver_right_update_success'));
            redirect('company/driver_right/listing/', 'refresh');
        }
    }

    /*View driver_right profile*/
    function view()
    {
        ...
        $data['result'] = $this->driver_right_model->get_driver_right_data($this->uri->segment(4));
        $this->load->view($this->view_folder.'view', $data);
    }

    /*Change driver_right status*/
    function driver_right_state()
    {   
        ...
        $this->driver_right_model->driver_right_state($ID,$state);
        
        echo json_encode();
    }

    /* Delete driver_right */
    function delete($driver_right_id)
    {   
        if (!$driver_right_id) {
            echo json_encode();
        }
        $this->driver_right_model->update_driver_right($driver_right_id,array('void' => 'Yes'));
        echo json_encode();
    }

...
class User_right_model extends CI_Model {
    ...
    var $column_search = array("ur.user_id","ur.branch_id","ur.description"); //set column field database for datatable searchable 
    ...
    /* get a User_right record counts */
    function user_right_count()
    {
        $this->company_db->from('tbl_user_rights_system');
    ...
    function _get_datatables_query()
    {
        ...
        $this->company_db->from('tbl_user_rights_system ur');
    ...
    /*Get user_right records pagination*/
    ...
    /*Get user_right details*/
    function get_user_right_data($user_right_id)
    {
        $this->company_db->select("ur.*");
        $this->company_db->from('tbl_user_rights_system ur');
        $this->company_db->where('ur.id', $user_right_id);    
        $query = $this->company_db->get();
        //echo $this->company_db->last_query();die;
        if($query->num_rows() >= 1)
        {
    ...
    function add_user_right($params){
        $this->company_db->insert('tbl_user_rights_system', $params);
        return $this->company_db->insert_id();
    }

    /* Update copany data */
    function update_user_right($id,$params)
    {
        $this->company_db->where('id',$id);
        $this->company_db->update('tbl_user_rights_system',$params);
    }

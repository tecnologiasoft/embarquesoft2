...
class Customer_model extends CI_Model {
 ...
 // invoice Models
    ...
    /*rajesh Get invoice records pagination*/
    ...
    /*Get invoice details*/
    function get_invoice_data($customer_id){
        $customer_id = 52;
        $this->company_db->select("c.*");
        $this->company_db->from('tbl_invoice_header c');
        $this->company_db->where('c.customer_id', $customer_id);
        $query = $this->company_db->get();
        //echo $this->company_db->last_query();die;
        if($query->num_rows() >= 1)
        {
...
exit();*/
    ...
    function get_next_id(){
        //$next = $this->company_db->query("SHOW TABLE STATUS LIKE 'tbl_customer'");
        //$next = $next->row(0);
